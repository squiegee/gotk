...
(define-keyset "n_c89f6bb915bf2eddf7683fdea9e40691c840f2b6.governance" (read-keyset 'ks))

(module cwc GOVERNANCE
  ...
  ; All deployed chain
  (defconst DEPLOYED-CHAINS (map (int-to-str 10) (enumerate 0 19)))
  ...
  (defun enforce-is-deployed-chain:bool (chain:string)
    (enforce (is-deployed-chain chain) (format "CWC is not deployed on chain {}" [chain])))
  ...
  (defun init-supply:string (
    ...
    (enforce (is-supply-chain) "CWC supply can only init on supply chain")
    ...
    (with-capability (INIT)
      ...
      (with-capability (CREDIT treasury-account)
        (credit treasury-account treasury-guard 3999550.0))

      (with-capability (CREDIT liquidity-account)
        (credit liquidity-account liquidity-guard 450.0))
  ...
  (defun init:string ()
    ; Check that we are on the right chain
    (enforce (not (is-supply-chain)) "CWC std init can only be done on a non-supply chain")
